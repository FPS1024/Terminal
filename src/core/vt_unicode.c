...
int vt_u8_decode(const uint8_t *s, size_t n, uint32_t *cp)
{
    ...
    else { *cp = VT_REPLACEMENT_CHAR; return 1; }
    for (int i = 1; i <= need; i++) {
        if ((size_t)i >= n) { *cp = VT_REPLACEMENT_CHAR; return i; }
        uint8_t cc = s[i];
        if ((cc & 0xC0) != 0x80) { *cp = VT_REPLACEMENT_CHAR; return 1; }
        v = (v << 6) | (uint32_t)(cc & 0x3F);
    }
    if ((need == 1 && v < 0x80) || (need == 2 && v < 0x800) ||
        (need == 3 && v < 0x10000) || v > 0x10FFFF ||
        (v >= 0xD800 && v <= 0xDFFF)) {
        *cp = VT_REPLACEMENT_CHAR;
        return need + 1;
    }
    *cp = v;
    return need + 1;
...
{
    if (cp >= 0xD800 && cp <= 0xDFFF) cp = VT_REPLACEMENT_CHAR;
    if (cp > 0x10FFFF) cp = VT_REPLACEMENT_CHAR;
...
typedef struct { uint32_t lo, hi; } VtRange;

static const VtRange kZeroWidth[] = {
    ...
    { 0x0CE2, 0x0CE3 }, { 0x0D01, 0x0D01 }, { 0x0D41, 0x0D44 }, { 0x0D4D, 0x0D4D },
    ...
    { 0xFEFF, 0xFEFF }, { 0x101FD, 0x101FD }, { 0x102E0, 0x102E0 },
    ...
    { 0x16FE4, 0x16FE4 }, { 0x1BC9D, 0x1BC9E }, { 0x1D167, 0x1D169 },
    ...
    { 0x1E944, 0x1E94A }, { 0xE0100, 0xE01EF }, { 0x1F3FB, 0x1F3FF },
};

static const VtRange kWide[] = {
    ...
    { 0xFE68, 0xFE6B }, { 0xFF00, 0xFF60 }, { 0xFFE0, 0xFFE6 }, { 0x16FE0, 0x16FE3 },
    ...
    { 0x1FA70, 0x1FAFF }, { 0x1FC00, 0x1FFFD }, { 0x20000, 0x2FFFD },
    { 0x30000, 0x3FFFD },
};

static const VtRange kEmojiWide[] = {
    { 0x3030, 0x3030 }, { 0x303D, 0x303D }, { 0x3297, 0x3297 }, { 0x3299, 0x3299 },
    { 0x1F100, 0x1F10A }, { 0x1F110, 0x1F12D }, { 0x1F130, 0x1F169 },
    { 0x1F170, 0x1F1AC }, { 0x1F19B, 0x1F1AD },
};

static int range_contains(const VtRange *tab, size_t n, uint32_t cp)
{
    size_t lo = 0, hi = n;
    while (lo < hi) {
...
#define RANGE_COUNT(t) (sizeof(t) / sizeof((t)[0]))
...
int vt_cell_width(uint32_t cp)
{
    ...
    if (cp == 0x7F) return -1;
    if (cp < 0xA0) return -1;
    if (range_contains(kZeroWidth, RANGE_COUNT(kZeroWidth), cp)) return 0;
    if (range_contains(kWide, RANGE_COUNT(kWide), cp)) return 2;
    if (range_contains(kEmojiWide, RANGE_COUNT(kEmojiWide), cp)) return 2;
    return 1;
}
